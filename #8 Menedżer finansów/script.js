...
const checkCategory = (transaction) => {
    switch (transaction) {
        case '[ + ] Przychód':
            categoryIcon = 'f<i class="fas fa-money-bill-wawe"></i>';
            break;
        case '[ - ] Zakupy':
...
const deleteTransaction = (id) => {
    const trasactionToDelete = document.getElementById(id);
    const transactionAmount = parseFloat(trasactionToDelete.childNodes[3].innerText);
    const indexOfTransaction = moneyArr.indexOf(transactionAmount);

    moneyArr.splice(indexOfTransaction, 1);

    trasactionToDelete.classList.contains('income') ?
        incomeSection.removeChild(trasactionToDelete) :
        expensesSection.removeChild(trasactionToDelete);
